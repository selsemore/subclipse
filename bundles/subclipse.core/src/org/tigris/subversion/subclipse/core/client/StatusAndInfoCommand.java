...
public class StatusAndInfoCommand extends StatusCommand implements ISVNCommand {
  ...
  public StatusAndInfoCommand(
      ISVNLocalResource svnResource, boolean descend, boolean getAll, boolean contactServer) {
  ...
  protected void execute(final ISVNClientAdapter client, final IProgressMonitor monitor)
    ...
    monitor.worked(50);
    remoteStatuses =
        collectRemoteStatuses(getStatuses(), client, Policy.subMonitorFor(monitor, 50));
  }
  ...
  private RemoteResourceStatus[] collectRemoteStatuses(
      ISVNStatus[] statuses, ISVNClientAdapter client, final IProgressMonitor monitor) {
    ...
    try {
      ...
      for (int i = 0; i < statuses.length; i++) {
        ...
        if (SVNStatusKind.UNVERSIONED.equals(localTextStatus)
          ...
          else result[i] = new RemoteResourceStatus(statuses[i], getRevisionFor(statuses[i]));
        } else {

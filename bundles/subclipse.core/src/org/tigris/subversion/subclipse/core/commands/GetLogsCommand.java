...
public class GetLogsCommand implements ISVNCommand {
   ...
   * @param limit limit the number of log messages (if 0 or less no limit)
      ...
      if (callback == null) {
        ...
        if (remoteResource instanceof BaseResource) {
          ...
          ISVNLocalResource svnResource =
              SVNWorkspaceRoot.getSVNResourceFor(remoteResource.getResource());
          if (svnResource != null) {
            ...
            if (status != null && status.isCopied()) {
              ...
              if (copiedFromUrl != null) {
                ...
                GetRemoteResourceCommand getRemoteResourceCommand =
                    new GetRemoteResourceCommand(
                        remoteResource.getRepository(), copiedFromUrl, SVNRevision.HEAD);
          ...
          if (!logMessagesRetrieved)
            svnClient.getLogMessages(
                ((BaseResource) remoteResource).getFile(),
                pegRevision,
                revisionStart,
                revisionEnd,
                stopOnCopy,

/**
 * ***************************************************************************** Copyright (c) 2003,
 * 2004 Subclipse project and others. All rights reserved. This program and the accompanying
...
public class GetStatusCommand implements ISVNCommand {
  ...
  public GetStatusCommand(
      ISVNRepositoryLocation repository, IResource resource, boolean descend, boolean getAll) {
  ...
  private LocalResourceStatus[] convert(ISVNStatus[] statuses) {
    ...
    for (int i = 0; i < statuses.length; i++) {
      localStatuses[i] =
          new LocalResourceStatus(statuses[i], getURL(statuses[i]), checkForReadOnly);
    }

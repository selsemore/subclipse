...
public interface IResourceStateChangeListener extends EventListener {
   ...
   *     <p>[Note: The changed state event is purposely vague. For now it is only a hint to
   *     listeners that they should query the provider to determine the resources new sync info.]
   */
  public void resourceSyncInfoChanged(IResource[] changedResources);

  /**
   * Notifies this listener that the resource's have been modified. This doesn't necessarily mean
   * that the resource state isModified. The listener must check the state.
   *
  ...
  /**
   * Notifies this listener that the project has just been deconfigured and no longer has the SVN
   * nature.
   *
   * @param project The project that has just been configured
   */

...
/**
 * This class manages svn operations. beginOperation must be called before a batch of svn operations
 * and endOperation after
 *
 * <p>All changed .svn directories are refreshed using resource.refreshLocal SyncFileChangeListener
 * will then find that some meta files have changed and will refresh the corresponding resources.
 */
public class OperationManager implements ISVNNotifyListener {
  ...
  /** Begins a batch of operations. Forward notifications to messageNotifyListener */
  public void beginOperation(
      ISVNClientAdapter aSvnClient, OperationProgressNotifyListener anOperationNotifyListener) {
  ...
  public void endOperation(boolean refresh, Set<IResource> refreshResourceList)
      throws SVNException {
    endOperation(refresh, refreshResourceList, true);
  }

  /**
   * Ends a batch of operations. Pending changes are committed only when the number of calls to
   * endOperation() balances those to beginOperation().
   */
  public void endOperation(
      boolean refresh, Set<IResource> refreshResourceList, boolean refreshLocal)
      throws SVNException {
    try {
            ...
            if (resource.getProject().isOpen()) {
              try {
                SVNProviderPlugin.getPlugin()
                    ...
                    .refreshStatus((IContainer) resource, true);
              } catch (Exception e) {
  ...
  public void onNotify(File path, SVNNodeKind kind) {
    ...
    if (operationNotifyListener != null) {
      ...
      if ((operationNotifyListener.getMonitor() != null)
          && (operationNotifyListener.getMonitor().isCanceled())) {
        try {
